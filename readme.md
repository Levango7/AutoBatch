...
**AutoBatch** —— 可运行、可追溯、可扩展的大数据批处理工作流，覆盖数据接入、质量校验、清洗、计算、输出全流程，内置批次台账、质量规则引擎、失败定位与可视化看板。

## 特性

- **零第三方依赖**：仅用 Python 3.9+ 标准库（csv / json / hashlib / statistics / logging），无安装步骤；可选启用 Polars 列式加速（见下条，`pip install polars`）
- **一键端到端运行**：自动生成示例数据（2 万订单 + 3 千客户 + 200 商品，含 8 类缺陷）并完成五阶段处理
- **可追溯**：每次运行独立批次号（batch_id），全部中间产物持久化，台账（manifest.json）记录每个文件的 sha256、行数、来源标记与血缘关系
...
- **CI**：.github/workflows/ci.yml 在 push/PR 时自动跑 pytest 全量 + 流水线冒烟（Python 3.11/3.13 矩阵）
...
#    b. config 里改：
#       engine.spark.master="spark://localhost:7077"
...
```
# 1. 安装依赖（一次性）
pip install pyarrow minio        # pyarrow 14+（Parquet 读写 + S3 客户端）+ minio 7+（bucket 初始化）
...
# 调用 helpers.read_history_snapshot(table_name, snapshot_id) 读指定 snapshot 的历史数据
# 调用 helpers.list_snapshots(table_name) 列出全部 snapshot 及其时间戳 / 操作 / summary

# 6. 可选：零数据拷贝迁移现有 Parquet 表到 Iceberg
python tools/parquet_to_iceberg_migrate.py --parquet-dir run/latest/04_aggregates --table autobatch.aggregates
...
#    b. 构建 Spark 集群镜像时启用 Iceberg JAR（因 Iceberg JAR 最高支持 Spark 4.1 而 Docker 用 Spark 4.2，需显式开关）：
pwsh docker/spark-cluster/up.ps1 -BuildArg @("--build-arg", "ENABLE_ICEBERG=true")
...
- **polars 路径**（`engine.backend="polars"`）：需 `pip install polars`（Polars 1.43+，Rust 内核，wheel 约 30MB）。`polars` 采用 lazy import，未安装时仅 `backend="polars"` 路径报 `ImportError`，python 路径不受影响
...
- **parquet 湖存储路径**（`storage.backend="parquet"`）：需 `pip install pyarrow minio`（pyarrow 14+ 提供 Parquet 列式读写 + S3FileSystem 客户端，minio 7+ 提供 bucket 初始化与迁移脚本）。`pyarrow` / `minio` 采用 lazy import，未安装时仅 `storage.backend="parquet"` 路径报 `ImportError`，`storage.backend="local_csv"`（缺省）路径不受影响
...
- **spark + iceberg 三合一路径**（`engine.backend="spark"` + `storage.backend="iceberg"`）：需 `pip install pyspark "pyiceberg>=0.12.0rc1"` + JDK 11+ 或 17 + Iceberg REST catalog server。多机模式需 Docker 镜像构建时 `--build-arg ENABLE_ICEBERG=true` 打入 Iceberg spark-runtime JAR（因 Iceberg JAR 最高支持 Spark 4.1 而 Docker 用 Spark 4.2，缺省不打入）
...
│   ├── helpers.py           # 通用工具 + PipelineContext dataclass（阶段间上下文，含 state/incremental_enabled 字段）
...
├── tests/                   # pytest 测试套件（112 个用例）
│   ├── conftest.py          # 公共夹具（临时 run_dir / 配置 / inc_env 增量 / polars_env / parquet_env / s3_env / iceberg_env 隔离夹具）
│   ├── test_quality.py      # 8 类规则正反例 + referential 性能回归
│   ├── test_stages.py       # 5 个 stage 单测
│   ├── test_pipeline_e2e.py # 端到端冒烟（success / DQ Score / 血缘 / metrics.json）
│   ├── test_incremental.py  # 5 个增量场景（首次=全量 / 零增量 / 追加 / 失败重跑 / 全量回归）
│   └── test_engine_polars.py # Polars 等价性测试（4 个用例：全量等价 / DQ Score / 增量+Polars / Parquet）
│   └── test_engine_spark.py  # Spark 等价性测试（4 个用例：全量等价 / DQ Score / 增量+Spark / 多机模式 S3 等价性，Windows 缺 hadoop.dll 时 skipif，多机模式 Docker/MinIO 不可用时跳过）
...
├── .github/workflows/ci.yml # GitHub Actions CI（Python 3.11/3.13 矩阵，pytest + 冒烟）
...
└── requirements.txt         # 运行时零依赖（仅标准库）；可选依赖：polars / pyspark / pyarrow / minio / pyiceberg
```
...
- 各阶段耗时 158 / 1,524 / 673 / 858 / 1,117 ms（总 4,400 ms；validate 已做外键集合预计算优化，2 万行约 1.5 秒）
...
- **spark 路径**（`engine.backend="spark"`）：分布式加速，适合千万行以上、超单机内存上限、需多机并行、或已部署 Spark 集群的场景。本地模式 `master="local[*]"` 可单机跑通验证逻辑；多机模式 `master="spark://localhost:7077"` 通过 Docker Compose Standalone 集群（`docker/spark-cluster/up.ps1` 一键启动）+ MinIO 共享存储已可用（Phase 3 MinIO 已就位）
...
112 个用例（112 passed + 18 skipped + 0 failed，skip = 1 Polars Parquet + 3 Spark Windows 缺 hadoop.dll + 14 Iceberg/Spark+Iceberg 环境前置未就绪），覆盖：
...
- `test_incremental.py`：5 个增量场景（首次增量=全量 + 建水位、无新数据二跑零增量、追加新数据后只处理新增行且聚合 merge 正确、失败重跑幂等水位不推进、`enabled:false` 全量回归行为不变）
- `test_engine_polars.py`：4 个 Polars 等价性场景（全量产物与 python 路径一致、DQ Score in [0.95, 1.0] 且 lineage/metrics 正确、增量 + Polars 组合首次建水位+二跑零增量+追加只处理新增、Parquet 格式条件 skip）
- `test_engine_spark.py`：4 个 Spark 等价性场景（全量产物与 python 路径一致、DQ Score in [0.95, 1.0] 且 lineage/metrics 正确、增量 + Spark 组合首次建水位+二跑零增量+追加只处理新增、多机模式 S3 等价性 `test_cluster_spark_s3_equivalence` 验证 Docker Compose Standalone 集群产物与 python 路径一致；Windows 缺 `hadoop.dll` 时 `skipif` 跳过本地模式，多机模式在 Docker Desktop/MinIO 不可用时跳过）
...
`.github/workflows/ci.yml` 配置了 GitHub Actions：在 push 任意分支或提交 PR 时自动触发，于 ubuntu-latest 上以 Python 3.11 / 3.13 矩阵并行执行：

1. `pip install pytest`
2. `python -m pytest tests/ -v`（全量测试套件）
3. `python main.py --config config/pipeline_small.json`（流水线冒烟）
